...
df_list <- mclapply(1:60, load_df, mc.cores = 30)
rm(file_path)
...
dff <- mclapply(1:60, fix_df, mc.cores = 30) # put list into dff
rm(df_list)
...
dfSO <- mclapply(1:60, to_SO, mc.cores = 30)
rm(dff)
...
dfcoords <- mclapply(1:60, fill_in_coords, mc.cores = 30)
rm(dfSO)
...
dffin <- mclapply(1:60, sort_dfcoords, mc.cores = 30)
...
message("batch 1")
species_dists <- mclapply(1:60, make_sdist, mc.cores = 30)
#save file
saveRDS(species_dists, "~/mantel_files/exp3/result/sp_dist_exp3_1-60.rds")
rm(species_dists)
q()