...
setwd("~/mantel_files/no_mimic_runs/")
...
df_list <- mclapply(1:30, load_df, mc.cores = 30)
rm(file_path)
...
dff <- mclapply(1:30, fix_df, mc.cores = 30) # put list into dff
rm(df_list)
...
dfSO <- mclapply(1:30, to_SO, mc.cores = 30)
rm(dff)
...
dfcoords <- mclapply(1:30, fill_in_coords, mc.cores = 30)
rm(dfSO)
...
message("batch 1")
species_dists <- mclapply(1:30, make_sdist, mc.cores = 30)
#save file
saveRDS(species_dists, "~/mantel_files/no_mimic_result/sp_dist_nomimic_1-30.rds")
rm(species_dists)
q()