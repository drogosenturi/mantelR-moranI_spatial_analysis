##              Species distances for four nurseries, no mimicry            ##
message("start ", Sys.time())
#setwd("~/mantel_files/exp6/runs/")
setwd("~/soraida_r/raw_runs_7-10/exp6/runs/") #home desktop
...
df_list <- mclapply(1:10, load_df, mc.cores = 5)
rm(file_path)
...
dff <- mclapply(1:10, fix_df, mc.cores = 5) # put list into dff
rm(df_list)
...
dfSO <- mclapply(1:10, to_SO, mc.cores = 5)
rm(dff)
...
dfcoords <- mclapply(1:10, fill_in_coords, mc.cores = 5)
rm(dfSO)
...
dffin <- mclapply(1:10, sort_dfcoords, mc.cores = 5)
...
message("batch 1")
species_dists_1 <- mclapply(1:5, make_sdist, mc.cores = 5)
#save file
saveRDS(species_dists_1, "~/soraida_r/test/sp_dist_1.rds")
rm(species_dists_1)
species_dists_2 <- mclapply(6:10, make_sdist, mc.cores = 5)
saveRDS(species_dists_2, "~/soraida_r/test/sp_dist_2.rds")
rm(species_dists_2)
q()